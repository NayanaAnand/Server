...
PREPARE_LOGGING(Server_i)

Server_i::Server_i(const char *uuid, const char *label) :
    Server_base(uuid, label)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void Server_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &Server_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &Server_i::<callback method>)
        in the constructor.
        ...
        Server_i::Server_i(const char *uuid, const char *label) :
            Server_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &Server_i::scaleChanged);
            addPropertyListener(status, this, &Server_i::statusChanged);
        }

        void Server_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void Server_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
